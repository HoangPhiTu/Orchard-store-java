...
export default function EditCategoryPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: CategoryFormData) => categoryApi.update(categoryId, data as any),
    onSuccess: () => {
  ...
  // Build flat list for parent selection (exclude current category and its children)
  const buildFlatList = (categories: any[], excludeId: number, level: number = 0): any[] => {
    let result: any[] = [];
  ...
  const flatCategories = categories && category
    ? buildFlatList(categories, categoryId)
    : [];
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          ...
          <Button onClick={() => router.push('/categories')} className="rounded-2xl">
            Quay lại danh sách
          </Button>
        </div>
      </div>
    ...
    <div className="space-y-8 pb-8">
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-2">
          ...
          <h1 className="text-3xl font-bold text-slate-900">Chỉnh Sửa Danh Mục</h1>
        </div>
        <Button
          variant="outline"
          onClick={() => router.back()}
            ...
            <Card className="border-0 rounded-3xl shadow-lg shadow-slate-200/50">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={category.parentId?.toString() || 'none'}
                    onValueChange={(value) =>
